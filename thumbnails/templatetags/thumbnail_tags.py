...
def resize_image(image, width, height, force=True):
    ...
    cache_directory =  ("/%s/resized_%d_%d" % (dirname, width, height)).replace("//","/")
    cache_path = cache_directory
    try:
        if not os.path.exists( MEDIA_ROOT + "/" + cache_path):
             os.mkdir( MEDIA_ROOT + "/" + cache_path )
    except OSError:     #this means there is no directory where the original image was meant to be -- image has been deleted? Quit
        return None
    
    #get full path to thumb
    thumb_filename = "%d_%d_%s" % (width, height, path.split("/")[-1])
    path_to_thumb = MEDIA_ROOT + "/" + cache_path + "/" + thumb_filename
    ...
    path_to_original = MEDIA_ROOT + "/" + path
